...
async function processFileForParameters(port, file, dirname, src, extension) {
    const fileParts = file.split('.');
    ...
    if (isSvelteFile(file) && isFileParameters(file)) {
        ...
        const client = new ApolloClient({ uri: `http://localhost:${port}/graphql`, fetch: fetch$1 });
        // todo
...
async function processDirectory(config, dirname, src, extension = '') {
...
async function processDirectoryForParameters(port, config, dirname, src, extension = '', paramGeneratedFiles = []) {
    await asyncForEach(fs.readdirSync(src + extension), async (file) => {
        if (fs.statSync(src + extension + '/' + file).isDirectory()) {
...
async function readRoutes(port, config, app, dirname) {
    await fs.remove("jungle/build");
    await fs.ensureDir("jungle/build");
    await fs.copy("static", "jungle/build");
    const paramGeneratedFiles = await processDirectoryForParameters(port, config, dirname, "src/routes");
    ...
    app.use(express.static(join(dirname, "jungle/build")));
}
...
function startAppServer(app, port, callback, options = { liveReload: true }) {
    ...
    app.set("port", port);
    const appServer = http$1.createServer(app);
...
function startGraphqlServer(config, port, callback, graphiql) {
    ...
    const graphqlServer = http$1.createServer(graphqlExpressServer);
...
class JungleJs {
    constructor(params) {
        ...
        this.graphqlPort = normalizePort(params.graphqlPort || process.env.GRAPHQL_PORT || 3001);
